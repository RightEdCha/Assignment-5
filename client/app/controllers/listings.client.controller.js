  ...
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.update = function(isValid) {
       ...
       */
       ...
       /* Create the listing object */
       var listing = {
         name: $scope.name || document.getElementsByName('name')[0].placeholder,
         code: $scope.code || document.getElementsByName('code')[0].placeholder,
         address: $scope.address || document.getElementsByName('address')[0].placeholder
       };
      ...
      var id = $stateParams.listingId
       ...
       Listings.update(id, listing)
               .then(function(response) {
                 //if the object is successfully saved redirect back to the list page
                 $state.go('listings.list', { successMessage: 'Listing ID: ' + id + ' succesfully updated!' });
               }, function(error) {
                 //otherwise display the error
                 $scope.error = 'Unable to save listing!\n' + error;
               });
